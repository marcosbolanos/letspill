...
export const pillEvents = pgTable("pill_events", {
  ...
  pillDate: date("pill_date").notNull(),
  pillTaken: boolean("pill_taken").notNull()
},
  // Composite index on userId THEN pillDate, will speed up queries
  (table) => [
    index("pillEvents_userId_pillDate_idx").on(table.userId, table.pillDate, table.createdAt.desc())
  ]
)

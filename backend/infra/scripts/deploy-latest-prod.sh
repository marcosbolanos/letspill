...
REGION="eu-west-3"
CLUSTER="CommonProdCluster"
SERVICE_NAME=$(aws ecs list-services --cluster $CLUSTER --region $REGION --query 'serviceArns[?contains(@, `ProdLetspill`)]' --output text | awk -F'/' '{print $3}')

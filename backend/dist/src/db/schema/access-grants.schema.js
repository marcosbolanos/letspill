...
import { pgTable, text, timestamp, boolean, jsonb, index, unique, check } from 'drizzle-orm/pg-core';
...
export const accessGrants = pgTable("access_grants", {
    ...
    unique("access_owner_grantee_scope_uniq").on(table.ownerId, table.granteeId),
    check("no_self_grants", sql `${table.ownerId} != ${table.granteeId}`)
]);
export const accessGrantLog = pgTable("access_grant_log", {
    id: text("id").primaryKey(),
